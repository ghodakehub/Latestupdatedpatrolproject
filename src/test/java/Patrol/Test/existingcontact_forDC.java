...
public class existingcontact_forDC extends NewBaseTest{
	
@Test(description = "Verify All Contact tab on case details page for supreme court Cases")
@Severity(SeverityLevel.CRITICAL)
@Description("This test verifies can we add existing contact in case details page successfully")
public void verifyexistingcontact() throws InterruptedException, IOException, MessagingException {
	try {
    ...
    contact.createcontact("Contact", "Existing Contact");
    contact.existingcontact("pratiksha");
   
    boolean verifyNotes =contact.verifyaddcontact("pratiksha");
    ...
    if (!verifyNotes) {
      ...
      Library.errorUrls.add(url);
        ...
        String message = "please check Issue coming while create existing contact . Please check the attached report.";
